 ...
 * receives raw print jobs over Wi-Fi and transfers data bytes securely across
...
// --- Peripherals & Indicator Output Pins ---
 ...
 *
 * @note DESIGN CHOICE: Called strictly AFTER Wi-Fi initialisation succeeds to
 * ensure no IO pin-state transients disrupt CYW43 radio initialisation.
 */
void init_hardware() {
	...
	for (uint32_t pin : inputs) {
		...
		gpio_disable_pulls(pin); // Rely on external pull-ups on PCB
	}

	// Auxiliary I2C Setup (Host Display / OLED telemetry)
...
/**
 * @brief LowIP TCP payload reception callback (Executes on Core 0 context).
 *
 ...
 *
 * Initialises stdio, Wi-Fi drivers, establishes network association,
...
int main() {
	...
	// Initialise the CYW43439 Wi-Fi hardware stack FIRST
	...
	// Disable power-save sleep modes to optimize DHCP & TCP negotiation speed
	cyw43_wifi_pm(&cyw43_state, CYW43_NO_POWERSAVE_MODE);

	// Wi-Fi Association Retry Loop
	...
	while (err != 0 && retries < WIFI_MAX_TRIES) {
		...
		if (retries > 1) {
			...
			sleep_ms(2000); // Inter-attempt delay to allow AP state machine reset
		}
	...
	// Initialise parallel printer IO hardware after Wi-Fi stack is fully up
	init_hardware();
